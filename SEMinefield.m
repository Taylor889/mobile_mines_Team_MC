classdef SEMinefield < SEBase
    ...
    methods
        ...
        function reset(obj)
            % Determine which mine class to use
            switch lower(obj.mineType)
                case 'mobile'
                    mineClass = @SEMobileMine;
                case 'static'
                    mineClass = @SEStaticMine;
                otherwise
                    warning('Unrecognized mine type ''%s'', using mobile mines', obj.mineType);
                    mineClass = @SEMobileMine;
            end

            % Preallocate mine array
            obj.mines = SEMine.empty(obj.number_of_mines, 0);

            for n = 1:obj.number_of_mines
                % Initialize each mine at (1,1)
                if isempty(obj.axes_h)
                    obj.mines(n) = mineClass(1, 1); % no axes yet
                else
                    obj.mines(n) = mineClass(1, 1, obj.axes_h); % use the GUI axes
                end

                % Attach axes if available
                if ~isempty(obj.axes_h)
                    obj.mines(n).setAxesHandle(obj.axes_h);
                end
            end

            % Now update mine positions according to the layout
            obj.resetLayout();
        end
